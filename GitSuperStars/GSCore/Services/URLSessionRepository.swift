...
public class URLSessionRepository: Repository {
    ...
    public func getSwiftRepositories(page: Int, perPage: Int, completion: @escaping (Result<GitRepositoryQueryResult>) -> Void) {

        print("GSCore:: Loading page:\(page) perPage:\(perPage)")
        ...
        guard !xRatingLimit.isLimitReached else {
            ...
            print("GSCore:: Loading error:\(errorMessage)")
        ...
        URLSession.shared.dataTask(with: url) { [unowned self] (data, response, error) in
            ...
            if let error = error {
                DispatchQueue.main.async {
                    print("GSCore:: Loading error:\(error.localizedDescription)")
            ...
            guard let data = data else {
                DispatchQueue.main.async {
                    print("GSCore:: Loading unexpected error:\(LocalizedString.unexpectedNetworkError.value)")
            ...
            do {
               let queryResult = try JSONDecoder().decode(GitRepositoryQueryResult.self, from: data)
                DispatchQueue.main.async {
                    print("GSCore:: Loading success:\(queryResult.items.count)")
    ...
    private func calculateXRateLimiting() -> (isLimitReached: Bool, secondsRemainingToReset: Double) {
...
//        return (isLimitReached: isLimitReached, secondsRemainingToReset: msRemainingToReset/1000.0)
        return (isLimitReached: false, secondsRemainingToReset: 1)
    }
